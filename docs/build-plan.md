...
**Implemented; activation pending.** `tools.evaluate` and the live CI job are
in the repository. Phase 1 deployment plus one reviewed `--accept-baseline`
run are still required before the check can be made required without failing
every PR. This is deliberate: the old five-second p95 threshold already fails
known-correct calls, and a made-up replacement would be a less honest gate.
